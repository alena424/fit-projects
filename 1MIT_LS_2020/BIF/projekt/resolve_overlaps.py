...
def process_one_group(sortedSequences):
    ...
    arrayM = [0.0]
    i = 0
    finish = [task["end"] for task in sortedSequences]

    for infoSequence in sortedSequences:
        weightArray.append(infoSequence["score"])

        idx = bisect.bisect(finish, infoSequence["start"])
        indexEndBeforeStartArray.append(idx)

        if (i >= 0):
            newValue = weightArray[i + 1] + arrayM[indexEndBeforeStartArray[i + 1]]
        ...
        i = i + 1
        #print(indexEndBeforeStartArray)

    arrayS = []
    j = len(arrayM) - 1
    while j > 0:
        if (weightArray[j] + arrayM[indexEndBeforeStartArray[j]]) >= arrayM[j - 1]:
    ...
    with open(file_name, 'r') as inputFile:
        for line in inputFile:
            if line[0:2] == "##":  # We dont need comment in the array
                continue
            splitLine = [splits for splits in line.split("\t") if splits is not ""]              # making an array to work better with the structure
...
def run_algorithm(file_name):
    ...
    sequence_array = process_file(file_name)                                    # sequence array has important structure where index is (seqId + type + strand)

    for index, sequence_group in sequence_array.items():
        sortedSequences = sorted(sequence_group["data"], key=itemgetter("end"))  # sorting by end time, end time is in data attribute of the structure
        arrayS = process_one_group(sortedSequences)                              # processing each group and picking non-overlapping sequences with highest score
