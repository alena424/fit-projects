...
void vs_test(GPUVertexShaderOutput* const      output,
  ...
  if (vsInvocationCounter < 3) {
    ...
    memcpy(pullerOutputs + vsInvocationCounter, input->attributes,
...
TEST_CASE(
  ...
  VertexIndex const            indices[] = {0, 1, 4, 4, 1, 5, 1, 2, 5,
                                 5, 2, 6, 2, 3, 6, 6, 3, 7};
  puller.indices = indices;
  for (size_t a = 0; a < MAX_ATTRIBUTES; ++a) puller.heads[a].enabled = 0;
...
TEST_CASE("gpu_runPrimitiveAssembly should construct primitive.") {
  ...
  gpu_runPrimitiveAssembly(gpu, &primitive, nofPrimitiveVertices, &puller,
...
TEST_CASE(
  ...
  for (size_t v = 0; v < VERTICES_PER_TRIANGLE; ++v)
    construct2DLine(lines + v, vertices + v,
...
TEST_CASE(
  ...
  cpu_uniformMatrix4fv(gpu, getUniformLocation(gpu, "viewMatrix"),
                       (float*)&viewMatrix);
  cpu_uniformMatrix4fv(gpu, getUniformLocation(gpu, "projectionMatrix"),
  ...
  cpu_setVertexPullerHead(gpu, puller, 1, vbo, sizeof(float) * 3,
                          sizeof(float) * 3);
...
TEST_CASE("phong_fragmentShader should compute phong color.") {
  ...
  // upload uniform data
  cpu_uniform3f(gpu, getUniformLocation(gpu, "cameraPosition"), 0.f, 20.f,
                20.f);
...
TEST_CASE("SOLUTION_TEST: gpu_runVertexPuller should construct vertex") {
  GPUVertexPullerConfiguration puller;
  VertexIndex const            i[] = {1, 2, 3, 4,  3,  2, 1, 4, 5,
                           6, 7, 8, 12, 11, 3, 4, 1, 7};
  puller.indices = i;
  for (size_t a = 0; a < MAX_ATTRIBUTES; ++a) puller.heads[a].enabled = 0;
