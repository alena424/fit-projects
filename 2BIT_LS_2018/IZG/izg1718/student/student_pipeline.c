...
VertexIndex gpu_computeGLVertexID(
  ...
  if ( indices == NULL ){
    // buffer indexu je null, indexing neni aktivni, index vertexu => poradove cislo invokace vertex shaderu
...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  return (void const*)((size_t)head->buffer + (size_t)head->offset + head->stride * gl_VertexID);
  //(void)head;
  //(void)gl_VertexID;
  //return NULL;
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  for (int i = 0; i < MAX_ATTRIBUTES; i++){
    // nastavime spravne adresy vertex atributu
    // hlava
    const GPUVertexPullerHead *puller_head = &puller->heads[i];

    if ( puller_head == NULL || puller_head->enabled ){
        // pro tento atribut neexistuje cteci hlava
        output->attributes[i] = NULL;
    } else {
        output->attributes[i] = gpu_computeVertexAttributeDataPointer(puller_head, gpu_computeGLVertexID(puller->indices, vertexShaderInvocation));
    }
  }
...
///  - GPUVertexShaderInput()
    ...
    GPUVertexPullerOutput *output = NULL;
    GPUVertexShaderInput *input = NULL;
    for (unsigned int i = 0; i < nofPrimitiveVertices; i++){
        // spustime vertex puller/vertex shader (to je ten co se stara o pripravu vrcholu - sada ctecich hlav)

        gpu_runVertexPuller(output,puller,baseVertexShaderInvocation+i);
        // vystupy ulozime do parametru primitive
        input->gl_VertexID = gpu_computeGLVertexID(puller->indices, baseVertexShaderInvocation+i);
        input->attributes = output;
        // ulozime to parametru primitive
        //primitive[i] = input;
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  // obsah celeho trojuhelnika
  //float obsah =

  // budeme pocitat obsahy malych trojuhelniku
  //vec2 vec_AB = vec2(vertices[1].data[0] - vertices[0].data[0], vertices[1].data[1] - vertices[0].data[1]);
  //Vec2 BC = Vec2(vertices[2].data[0] - vertices[1].data[0], vertices[2].data[1] - vertices[1].data[1]);

  //Vec2 AS = Vec2(pixelCenter[0].data[0] - vertices[0].data[0], pixelCenter[0].data[1] - vertices[0].data[1]);
  //Vec2 BS = Vec2(pixelCenter[0].data[0] - vertices[1].data[0], pixelCenter[0].data[1] - vertices[1].data[1]);
  //Vec2 CS = Vec2(pixelCenter[0].data[0] - vertices[2].data[0], pixelCenter[0].data[1] - vertices[2].data[1]);
