                     ...
                     const S_Material &material)
{
  ...
  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  // cos alfa = VxNx + VyNy + VzNz / |V|
  // CAMERA_DIST
  // v = ( a.x, a.y, -CAMERA_DIST - a.z)
  
  S_Coords v = S_Coords{aa.x, aa.y, -CAMERA_DIST - aa.z};
  double vectorabs = sqrt (v.x * v.x + v.y * v.y + v.z * v.z);
  double alfa = (v.x * nn.x + v.y*nn.y + v.z * nn.z) / vectorabs;
  if ( alfa < 0 ){
  	return;
  }
...
void DrawScene()
{
  ...
  // need to get matrix and change
  trSetMatrix(m);	

  trScale(-1.0,1.0,1.0); // pretocim x
  trTranslate(obj_move_x * 0.015, obj_move_y * 0.015, 0.0); // pounu od zrcadl
  trRotateX(obj_rot_x * 0.01); // natocim
  trRotateY(obj_rot_y * 0.01);
