...
class FakeEventsDatabase(private val context: Context) : EventsDatabase {
    ...
    private fun createEvent(
        ...
        isAllDay: Boolean,
        isCanceled: Boolean
    ) = Event(id, buildEventTitle(startTime), startTime, endTime, "Místnost $id", color, isAllDay, isCanceled)

    private fun buildEventTitle(time: Calendar): String {
        val sdf = SimpleDateFormat.getDateInstance(DateFormat.MEDIUM, Locale.GERMANY)
        val formattedDate = sdf.format(time.time)
        val hour = time.get(Calendar.HOUR_OF_DAY)
        val minute = time.get(Calendar.MINUTE)
        return String.format("Událost %02d:%02d %s", hour, minute, formattedDate)
    }
}
