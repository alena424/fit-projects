...
{
    char topChar;
    stackTop(s, &topChar); // nacteme znak ze zasobniku
    while ( ! stackEmpty(s) && topChar != '(' )
    {

        postExpr[*postLen ] = topChar; // nahraju prvni znak do postExpr
        *postLen+=1; // posunu se dal
        stackPop(s);
        stackTop(s, &topChar);
    }
    if ( topChar == '(' ) {
...
{
    /* pokud se zpracovava operator, muzu ho vlozit na zasobik v pripade ze:
    1. zasobnik je prazdny
    2. na vrcholu zasobniku je leva zavorka
    3. na vrcholu zasobniku je operator s nizsi prioritou

    Pokud je na zas. operator s vyssi nebo shodnou prioritou, ostranim ho, vlozim na vystup */
    ...
    // pushuju pouze pokud ostre mensi prirotita, pouze v pripade, ze mam na zasobniku +,- a vkladam neco jineho nez +,- (napr. *, /)
...
char* infix2postfix (const char* infExpr)
{
    ...
    int unsigned index = 0; // pro indexovani pole
    int unsigned i = 0; // index vstupniho souboru
// inicializuju zasobnik
    tStack *s;
...
// nas retezec malokuju
    char *postExpr;
    if ( ( postExpr = (char *) malloc(sizeof(char)* MAX_LEN) ) == NULL)
    {
        return NULL;
    }

//budu zpracovavat vstup
    while ( infExpr[i] != '\0'  )
    {
        if ( (infExpr[i] >= 'a' &&infExpr[i] <= 'z') ||
                ( infExpr[i] >= 'A' && infExpr[i] <= 'Z') ||
                ( infExpr[i] >= 0 && infExpr[i] <= 9 ) ||
                ( infExpr[i] >= '0' && infExpr[i] <= '9' ))
        {
            // pokud se jedna o alfanumericky znak, hodim ho na vystup
            postExpr[index] = infExpr[i];
            index++;
        }
        else if ( infExpr[i] == ')' )
        {
            //pokud narazime na pravou zavorku, vkladame hodnoty zasobniku na vystup,
            // dokud nenarazime na levou zavorku
            untilLeftPar(s,postExpr,&index);
        }
        else if ( infExpr[i] == '(' )
        {
            // pokud prijde leva zavorka, dame ji na vrchol zasobnika
            stackPush(s, infExpr[i]);
        }
        else if ( infExpr[i] == '='  )
        {
            //pokud jsme narazili na rovna se, ostranujeme prvky ze zasobniku a vkladame je na vystup
            // nakonec jeste pridame '='
            while ( ! stackEmpty(s))
            {

                stackTop(s, &postExpr[index]); // nacteme na vystup
                stackPop(s); // odstranime prvek
                index++;
            }
            postExpr[index] = '=';
            index++;
        ...
        {
            // jedna se o operand
            doOperation(s,infExpr[i],postExpr,&index);
        }
        i++;
    }
    free(s);
