...
/*                                                                                  */
/*  Reseni vytvoril(a) a odevzdava: (Jiri Matejka, xmatej52)                                  */
/*                                                                                  */
/************************************************************************************/
...
void PORTB_IRQHandler(void) {
	...
	if (PORTB->ISFR & (1 << 3)) { /* tlacitko NAHORU (Up) */
		/* zacatek odezvy */
		display_msg(activation_mask(DIG2_ID + DIG3_ID), SEG_DIR_U);
		delay(timeout_display);
		clear_display();
		/* konec odezvy */
		PORTB->ISFR = ( PORTB->ISFR | (1 << 3) );
	}
	else if (PORTB->ISFR & (1 << 4)) { /* tlacitko PROSTREDNI (Central) */
		/* zacatek odezvy */
		...
		clear_display();
		/* konec odezvy */
		PORTB->ISFR = ( PORTB->ISFR | (1 << 4) );
	}
	else if (PORTB->ISFR & (1 << 5)) { /* tlacitko DOLEVA (Left) */
		/* zacatek odezvy */
		display_msg(activation_mask(DIG1_ID), SEG_DIR_L);
		delay(timeout_display);
		clear_display();
		/* konec odezvy */
		PORTB->ISFR = ( PORTB->ISFR | (1 << 5) );
	}
	else if (PORTB->ISFR & (1 << 6)) { /* tlacitko DOLU (Down) */
		/* zacatek odezvy */
		display_msg(activation_mask(DIG2_ID + DIG3_ID), SEG_DIR_D);
		delay(timeout_display);
		clear_display();
		/* konec odezvy */
		PORTB->ISFR = ( PORTB->ISFR | (1 << 6) );
	}
	else if (PORTB->ISFR & (1 << 7)) { /* tlacitko DOPRAVA (Right) */
		/* Zacatek odezvy */
		display_msg(activation_mask(DIG4_ID), SEG_DIR_R);
		delay(timeout_display);
		clear_display();
		/* konec odezvy */
		PORTB->ISFR = ( PORTB->ISFR | (1 << 7) );
	}
	else { }
}
...
void init_hardware(void) {
	 ...
	 */
	for (int i = 3; i < 8; i++) { // tlacitka 3-7
		PORTB->PCR[i] = ( PORT_PCR_ISF(i) | PORT_PCR_MUX(1) | PORT_PCR_PS(1) | PORT_PCR_PE(1) | PORT_PCR_IRQC(10) );
 	}
